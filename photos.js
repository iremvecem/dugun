// FOTO_YUKLEME_SURUMU: 20260716-upload-v3
...
if (!elements.app) throw new Error("Fotoğraf bölümü bulunamadı.");
...
async function startPhotoApp() {
  ...
  const { doc, getDoc, getFirestore, runTransaction, serverTimestamp, setDoc } = firestoreApi;
  const { deleteObject, getStorage, ref, uploadBytesResumable } = storageApi;
  ...
  const state = { user: null, uploadCount: 0, uploading: false };
  ...
  // Klavyeyle erişim; telefonda label doğrudan yerel dosya/kamera seçicisini açar.
  for (const [control, input] of [
    [elements.cameraButton, elements.cameraInput],
    [elements.galleryButton, elements.galleryInput]
  ]) {
  ...
  setUploadControlsDisabled(true);
  showStatus("Fotoğraf yükleme hazırlanıyor…", "info");
  updateWindowState();
  ...
  function updateWindowState() {
    ...
    if (now < start) {
      ...
      elements.closed.hidden = false;
      elements.closed.textContent = "Fotoğraf yükleme süresi sona erdi.";
  ...
  async function consumeSelection(input) {
    ...
    if (!state.user) {
      showStatus("Bağlantı henüz hazır değil. Birkaç saniye sonra tekrar deneyin.", "error");
      return;
    }
    ...
    for (const file of acceptedFiles) {
      ...
      try {
        if (!file.type.startsWith("image/")) throw new Error("Bu dosya bir fotoğraf değil.");
    ...
    showStatus(
      successCount > 0
        ? `${successCount} fotoğraf başarıyla İrem ve Cem’e ulaştı.`
  ...
  async function reserveSlot(uploaderName) {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      const data = { uploadCount: slotNumber, uploaderName, updatedAt: serverTimestamp() };
      if (snapshot.exists()) transaction.update(uploaderRef, data);
      else transaction.set(uploaderRef, data);
    });
...
function createQueueRow(filename) {
  ...
  return {
    ...
    setProgress(value) { bar.style.width = `${Math.max(0, Math.min(100, value))}%`; }
  };
}
...
function uploadWithProgress(storageRef, blob, uploaderName, slot, onProgress) {
  return new Promise((resolve, reject) => {
    const task = uploadBytesResumableFn(storageRef, blob, {
      ...
      customMetadata: { uploaderName, slot }
    });
...
async function decodeImage(file) {
  if ("createImageBitmap" in window) {
    ...
    catch (_) { /* Safari/HEIC için Image yöntemi */ }
  }
